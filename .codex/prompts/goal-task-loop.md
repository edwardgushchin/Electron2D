...
CHECKS: сначала минимальные релевантные проверки, затем `git diff --check`. Для мелкой правки аудиторского запроса, локального prompt-а или разбора отчёта сначала `dotnet run --project eng/Electron2D.Build -- verify audit-contracts`. При source changes: `dotnet run --project eng/Electron2D.Build -- verify licenses`. Перед архивом задачи после triage: `dotnet run --project eng/Electron2D.Build -- verify audit-followups`.
