...
- Task state: `.taskboard/board.e2tasks` and `.taskboard/{tasks,completed}/*.e2task`, accessed only through `e2d tasks`.
...
1. PREFLIGHT: load the sources above for `<task-id>`, read the task and board through `e2d tasks get <task-id> --project . --format json` and `e2d tasks board --project . --format json`, inspect relevant saved verdict reports and diary notes, then identify current HEAD, baseline, scope, acceptance criteria and planned checks.
2. READY-TO-START: continue to work only if task state is `open` or an explicitly defined ready state, all dependencies are accepted/closed or have saved `ACCEPT`, and each external blocker has an explicit current user override. If state is `blocked` or `tracking`, dependencies are not accepted, or an external blocker has no override, add a blocker report through `e2d tasks comment add`, set the task to `Blocked` through `e2d tasks set-status` when appropriate, update the diary, then stop without production work, audit packaging or commit.
...
8. ACCEPT/DONE: after accepted reports and closed actionable notes, sync task status and activity only through `e2d tasks`, archive accepted work through `e2d tasks archive` when explicitly authorized, update the diary/release notes as required, remove temporary audit artifacts, run `e2d tasks verify --project . --format json` and the remaining final checks, inspect `git status`, then create the requested commit without push.
...
Before preparing the next audit package, add a blocker synthesis to task activity through `e2d tasks comment add` and to the diary:
