...
6. PACKAGE/VERIFY/SUBMIT: when external acceptance is needed, follow `docs/release-management/audit-package.md` exactly. Repeated external blockers must first pass the audit-loop root-cause gate below, then move through the stabilization model, including `audit-loop-stabilization`, `previousVerdictChain` and `blockerClosureList`.
