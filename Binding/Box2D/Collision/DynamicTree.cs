...
using Electron2D.Binding.Box2D.Dynamics.Internal;
...
namespace Electron2D.Binding.Box2D.Collision
{
    public class DynamicTree
    {
        ...
        public void DestroyProxy(int proxyId)
        {
            ...
            System.Diagnostics.Debug.Assert(_treeNodes[proxyId].IsLeaf());
        ...
        /// </summary>
        public void Query(in ITreeQueryCallback callback, AABB aabb)
        {
        ...
        /// </summary>
        public void RayCast(in ITreeRayCastCallback inputCallback, RayCastInput input)
        {
            var p1 = input.P1;
            var p2 = input.P2;
            var r = p2 - p1;
            ...
            while (stack.Count > 0)
            {
                ...
                if (!CollisionUtils.TestOverlap(node.AABB, segmentAABB))
                {
                    continue;
                }
	...
	{
		System.Diagnostics.Debug.Assert(0 <= freeIndex && freeIndex < m_nodeCapacity);
		freeIndex = m_nodes[freeIndex].next;
		++freeCount;
	}

	System.Diagnostics.Debug.Assert(GetHeight() == ComputeHeight());

	System.Diagnostics.Debug.Assert(m_nodeCount + freeCount == m_nodeCapacity);
#endif
        }
        ...
        private void InsertLeaf(int leaf)
        {
            ...
            while (index != NullNode)
            {
                index = Balance(index);
                ref var indexNode = ref _treeNodes[index];
                System.Diagnostics.Debug.Assert(indexNode.Child1 != NullNode);
                System.Diagnostics.Debug.Assert(indexNode.Child2 != NullNode);
        ...
        private int Balance(int iA)
        {
            ...
            if (balance > 1)
            {
                ...
                if (C.Parent != NullNode)
                {
                    ...
                    {
                        System.Diagnostics.Debug.Assert(_treeNodes[C.Parent].Child2 == iA);
            ...
            if (balance < -1)
            {
                ...
                if (B.Parent != NullNode)
                {
                    ...
                    {
                        System.Diagnostics.Debug.Assert(_treeNodes[B.Parent].Child2 == iA);
        ...
        private void ValidateStructure(int index)
        {
            ...
            if (index == _root)
            {
                System.Diagnostics.Debug.Assert(_treeNodes[index].Parent == NullNode);
            }
            ...
            System.Diagnostics.Debug.Assert(_treeNodes[child1].Parent == index);
            System.Diagnostics.Debug.Assert(_treeNodes[child2].Parent == index);
