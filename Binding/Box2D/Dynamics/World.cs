...
namespace Electron2D.Binding.Box2D.Dynamics
{
    public class World : IDisposable
    {
        ...
        public bool DestroyBody(Body body)
        {
            ...
            System.Diagnostics.Debug.Assert(condition: !IsLocked);
            if (IsLocked)
            {
                return false;
            }
        ...
        {
            System.Diagnostics.Debug.Assert(!IsLocked);
            System.Diagnostics.Debug.Assert(JointList.Count > 0);
            if (IsLocked)
            {
                return;
            }
            ...
            bodyA.IsAwake = true;
            System.Diagnostics.Debug.Assert(bodyA.JointEdges.Count > 0);
            bodyA.JointEdges.Remove(joint.EdgeA.Node);
            joint.EdgeA.Dispose();

            var bodyB = joint.BodyB;
            bodyB.IsAwake = true;
            System.Diagnostics.Debug.Assert(bodyB.JointEdges.Count > 0);
        ...
        /// </summary>
        public void QueryAABB(in IQueryCallback callback, AABB aabb)
        {
        ...
        /// </summary>
        public void RayCast(in IRayCastCallback callback, Vector2 point1, Vector2 point2)
        {
        ...
        private void Solve(TimeStep step)
        {
            ...
            while (bodyNode != null)
            {
                ...
                bodyNode = bodyNode.Next;
                if ((body.Flags & BodyFlags.Island) != 0) // 已经分配到岛屿则跳过
                {
                    continue;
                }

                if (!body.IsAwake || !body.IsEnabled) // 跳过休眠物体
                {
                    continue;
                }
                ...
                while (stack.Count > 0)
                {
                    ...
                    //--stackCount;
                    var b = stack.Pop();
                    System.Diagnostics.Debug.Assert(b.IsEnabled);
                    island.Add(b);
                    ...
                    while (node != null)
                    {
                        ...
                        // 接触点已经标记岛屿,跳过
                        if ((contact.Flags & Contact.ContactFlag.IslandFlag) != 0)
                        {
                            continue;
                        }
        ...
        private void SolveTOI(TimeStep step)
        {
            ...
            for (;;)
            {
                ...
                while (contactNode != null)
                {
                    ...
                    var alpha = 1.0f;
                    if ((c.Flags & Contact.ContactFlag.ToiFlag) != 0)
                    {
                    ...
                    else
                    {
                        ...
                        System.Diagnostics.Debug.Assert(typeA == BodyType.DynamicBody || typeB == BodyType.DynamicBody);
        ...
        public void DebugDraw()
        {
            ...
            if ((flags & DrawFlag.DrawCenterOfMass) != 0)
            {
                var node = BodyList.First;
                while (node != null)
                {
