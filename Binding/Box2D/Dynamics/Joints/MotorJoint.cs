...
namespace Electron2D.Binding.Box2D.Dynamics.Joints
{
    ...
    public class MotorJoint : Joint
    {
        ...
        {
            System.Diagnostics.Debug.Assert(force.IsValid() && force >= 0.0f);
            _maxForce = force;
        }
        ...
        {
            System.Diagnostics.Debug.Assert(torque.IsValid() && torque >= 0.0f);
            _maxTorque = torque;
        }
        ...
        {
            System.Diagnostics.Debug.Assert(factor.IsValid() && 0.0f <= factor && factor <= 1.0f);
