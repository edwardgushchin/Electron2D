...
- After `T-0228`, do not assemble external audit ZIP files manually. The exact contract lives in `docs/release-management/audit-package.md`; use `dotnet run --project eng/Electron2D.Build -- audit package --task <task-id> --iteration rNN --baseline <sha> --config <path> --out .temp/audit`.
...
- Any repository task that an agent carries to acceptance after `T-0233` remains open until `audit submit` saves a complete external report whose first non-empty line is exactly `VERDICT: ACCEPT`. Local checks, manual page text, prompt examples, or stale chat history do not count as acceptance.
