...
- The exact external-audit contract lives in `docs/release-management/audit-package.md`. This section records guardrails only; it is not the package, browser, snapshot, or verdict protocol.
- Do not assemble, edit, repair, or verify audit ZIP files manually. Use `dotnet run --project eng/Electron2D.Build -- audit package ...`, then `dotnet run --project eng/Electron2D.Build -- audit package verify ...` on a separate clean checkout before any submission.
- Submit only with `dotnet run --project eng/Electron2D.Build -- audit submit ... --browser-backend codex-chrome` through the installed Codex Chrome Extension. Manual submission, manual page verdict reading, clipboard verdicts, copied cookies or session files, repo-local submit skills, and alternative browser paths are forbidden.
- External audit is an acceptance gate, not an inner debugging loop. Before packaging, run the narrow local checks required by `docs/release-management/audit-package.md`; if the same local failure class repeats twice, stop increasing `rNN` and stabilize it with a local driver or harness test before any new package or submit attempt.
- After the first `audit package` or `audit submit` in a task, keep the package scope frozen. New process ideas, adjacent tooling work, roadmap changes, or unrelated documentation edits must become follow-up work unless they are explicitly declared as combined scope in package metadata.
- After a saved primary `VERDICT: NEEDS_FIXES`, run the next primary iteration through the tool's reuse path. If the stored conversation is locally recorded as impossible to continue through the supported submit flow, use the supported `audit submit --new-conversation` path and record the reason in `TASKS.md` and the diary. If the command rejects missing or invalid conversation state, fix the state or inputs instead of bypassing it manually.
- After primary `VERDICT: ACCEPT`, do not change the accepted scope before control audit. Control audit starts a new ChatGPT project chat through `--control-audit`, not the primary conversation, and uses a clean-control ZIP without saved verdict reports or prior verdict/blocker context. First close non-blocking `RISKS_AND_NOTES` through tracked bookkeeping when possible; if code, documentation, tests, evidence, or accepted-scope configuration must change, create a new package and a new primary audit before control.
- After `T-0233`, task acceptance requires saved primary and control reports whose first non-empty line is exactly `VERDICT: ACCEPT`. Prompt text, chat preview, stale history, or manual page reading do not count.
- Audit ZIP files, generated audit manifests, raw evidence, clean checkouts, downloads, and `.temp/audit/**/conversation-url*.txt` are local working artifacts. Do not stage or commit them unless a task explicitly changes test fixtures or repository-owned audit-tool documentation.
- `TASKS.md` may name where local conversation state is stored, but must not store a concrete ChatGPT conversation URL as repo-owned evidence.
- If package creation, restore verification, or submit fails, do not send or accept the package. Fix the cause and create a new verified iteration.
