...
- After a saved primary `VERDICT: NEEDS_FIXES`, run the next primary iteration through the tool's reuse path. If the command rejects missing or invalid conversation state, fix the state or inputs instead of bypassing it manually.
- After primary `VERDICT: ACCEPT`, run an independent control audit with the same verified ZIP and no conversation reuse. If code, documentation, tests, evidence, or package configuration changed after primary `ACCEPT`, create a new package and a new primary audit first.
