...
- Outside `README.md`, public documentation must not explain Electron2D through SDL, SDL3, SDL_GPU, SDL_Renderer, SDL_ttf, SDL_mixer, SDL_shadercross, or companion library comparisons. In specifications, task files, completed-task archives, release notes, changelogs, GitHub Wiki sources, and implementation documentation, describe backend choices as internal platform, rendering, audio, text, or export backends without naming SDL-family projects unless the exact identifier is unavoidable in code, paths, package names, or a user-requested reference.
- Outside `README.md`, documentation must not call Electron2D, its runtime, or its public API "Godot-like", "Godot-подобный", or equivalent marketing language. If an internal task or specification must reference upstream API compatibility, use precise wording such as "совместимость с выбранным API-подмножеством Godot" and keep it technical, not promotional.
