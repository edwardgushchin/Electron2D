...
Synchronized with `docs/releases/0.1-preview.md`.
Последнее обновление: 2026-06-23.
Связанные документы: [Electron2D 0.1-preview](../releases/0.1-preview.md); [Agent-native cross-platform 2D game engine workflow Electron2D 0.1](agent-native-workflow.md); [Live ProjectWorkspace](../project-system/live-project-workspace.md).

## Canonical status

Этот документ больше не является самостоятельной старой целью проекта. Если исторический текст из `GOAL.md`, ранние architecture notes или локальные release drafts расходятся с `0.1-preview`, действующим источником является [Electron2D 0.1-preview](../releases/0.1-preview.md) и связанные спецификации.

Для `0.1-preview` canonical contract фиксирует:
...
- public runtime API строится только из утверждённого Godot 4.7 public API contract и не возвращает legacy/component API.
...
В публичных документах backend choices описываются как внутренние platform, rendering, audio, text, input, filesystem, physics и export backends. Конкретные библиотеки и wrappers являются implementation detail: они могут меняться без изменения public node/resource API, если сохраняется наблюдаемое поведение утверждённого Godot 4.7 public API contract.

## Подсистемы

| Подсистема | Canonical boundary для `0.1-preview` | Public API policy |
| --- | --- | --- |
...
| Платформа | Editor | Runtime | Export | Статус `0.1-preview` |
| --- | ---: | ---: | ---: | --- |
...
Публичный runtime API Electron2D `0.1-preview` воспроизводит утверждённую публичную поверхность Godot `4.7-stable` .NET/C# API под namespace `Electron2D`. Это техническая совместимость с Godot 4.7 public API contract; намеренные отличия допустимы только как `Deferred`/`Unsupported` через `T-0963`.
...
Remote Android/iOS/WebAssembly debugger, publishing automation, app store upload, cloud signing, remote hosting deploy и произвольное выполнение shell через MCP не входят в `0.1-preview`.
