...
- workspace строит один `EditorProjectTasksBoardSnapshot` из уже существующих источников: `ProjectTaskManager`, `TaskBoard`, `ProjectWorkspace` document revisions, `OperationJournal`, diagnostics, jobs и artifacts;
...
- панель строит один `EditorAgentWorkspacePanelSnapshot` из уже существующих источников: agent bootstrap state, `ProjectTaskManager`, `ProjectWorkspace` revisions, `OperationJournal`, `DiagnosticsStore`, `WorkspaceJob` и runtime artifacts;
