...
Архив: `data/completed-tasks/2026/06 Июнь.md#T-0114`.
Обновлено: 2026-06-23.
...
Ключевое обещание `0.1-preview`: `Electron2D.Editor` является основным живым рабочим пространством проекта. Разработчик может пользоваться редактором полностью вручную, но AI-агент видит актуальное состояние открытого проекта, выполняет семантически значимые операции редактора, запускает тестовый прогон, наблюдает результат и безопасно объединяет свои изменения с ручными правками.
...
Старые документы целей, если они описывают компонентную модель `SpriteRenderer`/`Rigidbody`/`AudioSource`, обязательный `Transform` у каждого `Node`, отсутствие script-binding, только четыре платформы без iOS или native-only contract без WebAssembly browser, считаются историческими, пока не синхронизированы с этой спецификацией и `docs/releases/0.1-preview.md`.
...
Godot 4 является каноническим UX- и layout-референсом для `Electron2D.Editor 0.1-preview`. Это означает знакомую структуру редактора: верхнее меню, переключатель центральных workspaces, scene/document tabs, `Scene` и `FileSystem` docks слева, `Inspector`/`Node` справа, bottom panel для output/debug/diagnostics и run controls в верхней зоне.
...
- `AssetLib` отсутствует в `0.1-preview`;
...
Полный UX-контракт описан в `docs/editor/godot4-editor-reference.md`.

Проверяемый минимум `T-0155` для `Tasks` workspace:

- workspace строит один `ProjectTasksBoardSnapshot` из уже существующих источников: `ProjectTaskManager`, `TaskBoard`, `ProjectWorkspace` document revisions, `OperationJournal`, diagnostics, jobs и artifacts;
- доска занимает центральную область выбранного workspace `Tasks` и использует общий workspace switcher `2D`, `Script`, `Game`, `Tasks`;
- видимые колонки идут в порядке `Backlog`, `Ready`, `In Progress`, `Blocked`, `Review`, `Awaiting Acceptance`, `Done`, `Cancelled` и содержат карточки с task id, title, priority, labels, assignee, readiness, blocking reasons, rank или sort key;
- выбранная карточка отображается в правом `Inspector` как `Task Details`, а не в Agent Workspace. Details показывают description, priority, labels, assignee, parent, dependencies, acceptance criteria, subtasks, activity, linked transactions, linked jobs, linked diagnostics, linked artifacts, screenshots, runtime artifacts и linked scenes/resources/nodes;
- drag-and-drop между колонками представлен как explicit move intent с target status, target rank, validation result и structured diagnostic при запрещённом transition;
- ручное изменение порядка карточек обновляет `Rank`/`SortKey` и сохраняет стабильный порядок после persistence round-trip;
- пользовательские действия включают create, edit, archive, hard delete, Accept, Request changes и Cancel. Hard delete требует отдельного destructive confirmation, если у задачи есть activity, dependencies или внешние ссылки;
- archived tasks скрываются в обычном board view, но доступны через archive view/filter;
- `Accept` и `Request changes` создают trusted interactive Editor `OperationContext`, то есть доверенный контекст действия человека внутри редактора; agent-originated payload не может подменить human acceptance;
- UI различает `Review`, `Awaiting Acceptance`, ручной `Status = Blocked`, `Readiness = BlockedByDependencies`, `Readiness = DependencyCancelled` и разные `BlockingReasons`; завершение dependency не снимает ручной blocker;
- board filters поддерживают status, priority, labels, assignee, text и linked object;
- board синхронизируется с workspace events: tooling/task mutation меняет snapshot revision, карточку и activity через тот же путь, которым пользуется ручной UI. Полная MCP-проверка остаётся задачей `T-0119`;
- board работает без активной AI- или MCP-сессии;
- UI smoke harness создаёт PNG screenshot и JSON analysis. Acceptance требует открыть screenshot, проверить центральное размещение board, правый `Task Details`, drag-and-drop affordances, кликабельные actions, отсутствие text overflow, отсутствие `3D`/`AssetLib`/GDScript UI и соответствие `docs/editor/godot4-editor-reference.md`.
...
Проверяемый минимум `T-0150` для Agent Workspace UI:

- панель строит один `AgentWorkspacePanelSnapshot` из уже существующих источников: agent bootstrap state, `ProjectTaskManager`, `ProjectWorkspace` revisions, `OperationJournal`, `DiagnosticsStore`, `WorkspaceJob` и runtime artifacts;
- snapshot показывает `AgentSessionId`, profile id, connection state, handshake state, route active Editor и последнее действие агента без вывода token, secret или temporary config path;
- блок текущей задачи показывает task id, статус, acceptance state, linked transactions, linked jobs, linked diagnostics, linked artifacts и признак human acceptance, то есть приёмки человеком;
- агентская команда может переводить задачу только в `Awaiting Acceptance`; action `Done`/acceptance не доступен в Agent Workspace для AI-контекста и остаётся человеческим действием;
- changeset разделяет изменённые объекты по типам: scenes, nodes, resources, scripts и settings. Каждый элемент содержит navigation target, то есть адрес для перехода к объекту в обычном UI редактора;
- diagnostics используют полный `StructuredDiagnostic` payload из общего diagnostics stream: `code`, `severity`, `message`, `location`, `relatedLocations` и `suggestedFixes` не теряются при отображении;
- artifacts показывают screenshots, runtime snapshots и job artifacts вместе с `InputSnapshotId`, `InputWorkspaceRevision`, `InputContentRevision`, `InputDocumentRevisions` и `InputBuildConfigurationHash`;
- job row показывает kind, state, progress, cancel availability, stale markers для import/build/test/export/run и состояние уже созданных artifacts;
- grouped Undo доступен только для обратимой последней AI-транзакции и содержит идентификатор undo group;
- cancel/stop controls активны только для текущей cancellable agent operation или run job;
- dock state фиксирует размещение справа под `Inspector`/`Node`, видимость в `2D`, `Script`, `Game` и `Tasks`, dockable/resizable/hideable/movable/maximizable behavior и сохранение layout;
- UI smoke harness создаёт PNG screenshot и JSON analysis. Acceptance требует открыть screenshot, проверить правое размещение dock, читаемость секций, кликабельные controls, отсутствие text overflow, недоступность AI-acceptance action и соответствие `docs/editor/godot4-editor-reference.md`.
...
`debug_attach` в `0.1-preview` не является произвольным `attach(pid)` для агента. Agent MCP/Tooling session может attach только к game process активной Editor play/debug session. Любой attach к другому процессу отсутствует либо требует явного интерактивного подтверждения разработчика.
...
Для JSON-представлений должна публиковаться JSON Schema. Целевая версия схемы для `0.1-preview` — JSON Schema Draft 2020-12.
...
Эталон для `0.1-preview` — Godot `4.7-stable` .NET/C# API. Electron2D реализует публичный C# API утверждённой поверхности под namespace `Electron2D` с полным compatibility evidence; GDScript не входит в публичный authoring path, а намеренные отличия требуют `Deferred`/`Unsupported` решений в manual public API profile.
...
- статус `supported`, `profile_approved`, `not_verified`, `out_of_profile`, `planned`;
- ссылку на Godot `4.7-stable` C# manifest для типов профиля;
- `parityEvidence` для явного разделения утверждённого профиля и ещё не доказанного strict parity;
...
Status: Supported / Profile approved
Out of profile: no
Parity evidence: not_verified
```

Для типов внутри release profile статусы `partial` и `experimental` недопустимы. Они могут существовать только для API вне обязательного профиля `0.1-preview` или для будущих расширений.

Команда `e2d api compare-godot <type> --format json` в текущей реализации является lookup по полному manual profile. Generated manifest используется отдельно для текущей runtime availability и parity evidence. Для строки `approved` команда подтверждает только решение владельца, а не доказывает полную строгую совместимость с Godot `4.7-stable`. Отдельный strict comparison gate остаётся будущим evidence-контрактом owning class tasks and final gates.

Для `deferred`/`unsupported` строк команда должна возвращать точное решение, для неизвестного имени - `type_not_found`; обходной API не рекомендуется.
...
- `sourcePath = data/api/electron2d-api-manifest.json` и `profileSourcePath = data/api/electron2d-public-api-profile.json`;
- `type.fullName`, `type.profile.decision`, `editorOnly`, `godotReference` и rationale взяты из manual profile; `type.id` и `type.availability` отражают наличие строки в текущем manifest;
- `result.status = profile_approved` для типа внутри утверждённого `0.1-preview` public API profile;
- `result.status = deferred|unsupported` для явно исключённой строки и `type_not_found` для неизвестного имени;
- `parityEvidence.status = not_verified`, пока отдельный strict Godot `4.7-stable` comparison evidence не создан owning class tasks and final gates.

Для `approved` строки команда завершается с `exitCode = 0`, `succeeded = true`, `route = none` даже до реализации типа, но явно ставит `type.availability.exported = false`; legacy strict-parity counter object не публикуется. Для `deferred`/`unsupported` строки и неизвестного типа команда завершается fail-closed: `exitCode = 1`, `succeeded = false`, `route = none`, точный `data.result.status` и stable diagnostic `E2D-CLI-0002`, без замены или обходного API.
...
`.taskboard/**` является `EditorMetadata`, а не игровыми ресурсами. Task documents:
...
`.taskboard/` отслеживается по умолчанию либо включается отдельной настройкой Project Manager; в обоих случаях template обязан сделать выбор явным.
...
Такой экспорт является отчётом, не обязателен для `0.1-preview` и не становится источником истины.
...
- команду `e2d api compare-godot <type>` как lookup утверждённого профиля и явное напоминание, что полная strict parity проверяется отдельным evidence-контрактом;
...
Документация каждого публичного API должна содержать назначение, сигнатуру, lifecycle restrictions, thread affinity, ownership/disposal, пример, ошибки, платформенные ограничения и renderer restrictions. Type-level отличия от Godot оформляются `Deferred`/`Unsupported` строками manual profile; намеренные member/enum subsets требуют `godotApiContract` с fail-closed default и явными decisions. Для типов внутри обязательной публичной поверхности текущий manifest фиксирует `profile_approved`, а полная strict parity требует отдельного evidence-контракта.
