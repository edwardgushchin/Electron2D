...
Старые документы целей, если они описывают компонентную модель `SpriteRenderer`/`Rigidbody`/`AudioSource`, обязательный `Transform` у каждого `Node`, отсутствие script-binding, только четыре платформы без iOS или native-only contract без WebAssembly browser, считаются историческими, пока не синхронизированы с этой спецификацией и `docs/releases/0.1.0-preview.md`.
