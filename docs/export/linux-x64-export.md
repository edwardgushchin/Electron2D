...
- создавать `electron2d-0.1-preview-linux-x64.tar.gz`;
...
- `linux-x64` with glibc is the only Linux runtime identifier verified in `0.1-preview`.
...
GitHub Actions запускает `dotnet run --project eng/Electron2D.Build -- package --rid linux-x64` только в `ubuntu-latest` job. Windows и macOS jobs продолжают запускать общий test runner и не пытаются публиковать `linux-x64`.
