...
В `0.1-preview` появился internal render queue для `CanvasItem` submissions. Public `CanvasItem`, `Node2D`, `Sprite2D` и `CanvasLayer` реализованы отдельным baseline и используют эту очередь через internal submission context.
