...
`RenderingProfile.Compatibility` - гарантированный минимальный профиль. Он активен по умолчанию для публичного профиля возможностей; интерактивный runtime может выбрать основной SDL GPU `presenter` или запасной `SDL_Renderer` `presenter` внутри этого профиля.
...
- `Compatibility` уже строит internal command plan. Начиная с `T-0219`, интерактивный runtime отправляет этот план через основной SDL GPU `presenter` или запасной `SDL_Renderer` `presenter`; PNG-снимок читается из активного пути показа кадра.
