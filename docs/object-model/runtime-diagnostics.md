...
Это важно для чистого Electron2D публичного API: временные вспомогательные типы диагностики не экспортируются из runtime assembly и не становятся обязательством совместимости для `0.1-preview`.
