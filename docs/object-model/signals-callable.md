...
Если callback бросает исключение или аргументы не подходят сигнатуре callable, emission продолжает обход остальных callbacks, но возвращает `Error.Failed`. Исключение из пользовательского signal callback дополнительно записывается во внутреннюю диагностику `SceneTree`, если для target или emitter известно дерево.
