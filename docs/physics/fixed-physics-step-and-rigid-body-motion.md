...
`0.1-preview` должен иметь предсказуемый начальный цикл физики:
...
`0.1-preview` теперь выполняет физику фиксированными шагами по `1/60` секунды и имеет начальное движение `RigidBody2D` по `LinearVelocity`.
