...
`RunSession` публикует `ProcessId` active game process. Managed debugger использует этот id только для attach к процессу игры, запущенному самим Editor; произвольный attach к чужим процессам не является частью baseline `0.1.0`.
