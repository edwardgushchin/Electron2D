...
```text
Ready | In Progress | Blocked | Review | Done | Cancelled
```
...
- Board показывает колонки `Ready`, `In Progress`, `Blocked`, `Review`, `Done`, `Cancelled`.
...
- Внутренняя проверка и ожидание пользовательской приёмки визуально различимы внутри `Review` через `acceptanceState`, а manual blocker и dependency blocker показаны разными markers.
