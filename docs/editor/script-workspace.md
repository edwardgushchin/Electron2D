...
- нижняя вкладка `Agent` с current task `T-0161`, transaction link, debug job link и screenshot artifact link.
...
- справа видны `Inspector`, `Code Document`, `Dirty Revision`, `Diagnostic`, `Conflict`;
- снизу видны панели `Diagnostics` и вкладка `Agent`;
...
- нижняя вкладка `Agent` показывает `T-0161`, transaction, job и screenshot artifact;
- текст не выходит за границы контейнеров и не перекрывает соседние элементы.
