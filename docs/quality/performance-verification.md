# Проверка производительности для `0.1-preview`
...
Обновлено: 2026-06-30.
Связанные документы: [Electron2D 0.1-preview](../releases/0.1-preview.md); [Бюджеты производительности и длительные критерии 0.1-preview](../release-management/performance-budgets.md); [Platformer](../examples/platformer.md).

## Цель

`0.1-preview` должен иметь локальную автоматическую проверку производительности для коротких, воспроизводимых сценариев. Проверка не заменяет 30-минутные длительные проверки из платформенного релизного контроля, но закрывает базовый контракт `T-0102`: 60 FPS для активного эталонного проекта, отсутствие постоянных управляемых выделений памяти .NET каждый кадр после прогрева и доказательство, что группировка команд (`batching`) уменьшает количество вызовов отрисовки.
...
Задача: `T-0102`, дополнение `T-0215`.
Обновлено: 2026-06-30.
Спецификация: [Проверка производительности для 0.1-preview](performance-verification.md).
...
Аудиторские и CI-доказательства могут использовать `.temp/audit-evidence/T-0215/...` как `--out` путь. Эти JSON-файлы остаются временными доказательствами запуска, не входят в commit и не заменяют отслеживаемый Git файл `data/quality/performance-reference-metrics.json`.
