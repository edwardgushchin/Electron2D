...
В `T-0072` существует только default `Master` bus. Пользовательские buses, mute, solo, volume routing и global volume относятся к `T-0074`.
...
- пользовательские buses, mute, solo, volume routing и audio effects;
