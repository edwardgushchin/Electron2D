...
Формат должен быть узким и проверяемым. Он не обязан сразу реализовать весь `ResourceLoader`/`ResourceSaver`, threaded loading, импорт и editor file dock. Эти части остаются задачами следующих этапов. Текущая задача должна дать устойчивую модель данных, на которую они смогут опереться.
