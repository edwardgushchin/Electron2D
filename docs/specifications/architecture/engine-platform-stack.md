...
- поддерживаемые платформы release contract: Windows, Linux, macOS, Android и iOS;
- desktop Editor только на Windows, Linux и macOS;
- Android и iOS как runtime/export platforms, а не платформы редактирования;
...
Android и iOS входят в release contract как runtime/export platforms, но их готовность подтверждается только после закрытия platform tasks. Документация может описывать target requirements, signing references и expected smoke checks, но не должна считать mobile export готовым release path до реальной проверки.

Remote Android/iOS debugger, publishing automation, app store upload, cloud signing и произвольное выполнение shell через MCP не входят в `0.1.0 Preview`.
