...
Проект не содержит `TASKS.md`, `dev-diary/` или `completed-tasks/`: эти файлы относятся только к рабочему процессу разработки самого репозитория и не должны попадать в пользовательские проекты. Для задач игры используется только `ProjectTaskManager` в `.electron2d/tasks/**`.
