...
`examples/platformer/` должен быть законченной приёмочной игрой `0.1-preview`, а не standalone demo, fixture или набором runtime files. Проект должен открываться и проверяться тем же форматом, который использует `Electron2D.Editor`: `Platformer.e2d`, `.csproj`, main scene, project settings, Input Map, export presets, ресурсы, C# scripts и `.taskboard/**`.
...
- `examples/platformer/.taskboard/board.e2tasks`;
- `examples/platformer/.taskboard/tasks/platformer-acceptance.e2task`;
- `examples/platformer/.taskboard/tasks/T-0166.e2task`;
- `examples/platformer/.taskboard/tasks/T-0221.e2task`;
- `examples/platformer/.taskboard/tasks/T-0222.e2task`;
- `examples/platformer/.taskboard/tasks/T-0223.e2task`;
- `examples/platformer/.taskboard/tasks/T-0225.e2task`.
...
Проект может использовать curated assets из `data/assets/reference-games/`, но не должен ссылаться на сетевые download steps, placeholder files, ignored local workflow files, `TASKS.md`, `data/dev-diary/` или `data/completed-tasks/`.

Рабочие задачи разработки самой игры хранятся в `examples/platformer/.taskboard/tasks/*.e2task`. На 2026-06-30 в проектную доску перенесены `T-0166`, `T-0221`, `T-0222`, `T-0223` и `T-0225`; корневой legacy `TASKS.md` больше не является источником истины для этих игровых задач. `platformer-acceptance.e2task` остаётся отдельной карточкой человеческой приёмки, но заблокирована до завершения перенесённых задач и не считается принятой.

## Verification

Нужен C#-маршрут `dotnet run --project eng/Electron2D.Build -- verify platformer`, который:
...
- проверяет `.taskboard/**` как first-class Editor metadata;
- проверяет, что board содержит `T-0222` в `Ready`, зависимые `T-0223`, `T-0225`, `T-0221`, `T-0166` в `Blocked`, а `platformer-acceptance` заблокирована до завершения tracking-задачи;
...
- проверяет, что package-planning/verifier steps не копируют `.taskboard/**` в runtime asset list.
...
Статус: реализованный reference project для `T-0094`, с проектной доской задач Platformer после `T-0234`.
Обновлено: 2026-06-30.
...
- `.taskboard/board.e2tasks` - ProjectTaskManager-доска проекта;
- `.taskboard/tasks/platformer-acceptance.e2task` - карточка человеческой приёмки, заблокированная до завершения рабочих задач Platformer;
- `.taskboard/tasks/T-0166.e2task`, `.taskboard/tasks/T-0221.e2task`, `.taskboard/tasks/T-0222.e2task`, `.taskboard/tasks/T-0223.e2task` и `.taskboard/tasks/T-0225.e2task` - рабочие задачи Platformer, перенесённые из корневого списка задач репозитория в проектный `ProjectTaskManager`.

Проект не содержит `TASKS.md`, `data/dev-diary/` или `data/completed-tasks/`: эти legacy-файлы относятся только к прежнему рабочему процессу разработки самого репозитория и не должны попадать в пользовательские проекты. Для задач игры используется только CLI-managed `ProjectTaskManager` в `.taskboard/**`.
...
```bash
dotnet run --project eng/Electron2D.Build -- verify platformer
```
...
- проверку project task-board: `T-0222` готова к работе, зависимые `T-0223`, `T-0225`, `T-0221`, `T-0166` заблокированы зависимостями, а `platformer-acceptance` возвращена на доработку до закрытия перенесённых задач;
...
- проверку, что WebAssembly package output не содержит `.taskboard/**`.
