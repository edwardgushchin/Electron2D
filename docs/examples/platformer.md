...
Рабочие задачи разработки самой игры хранятся в `examples/platformer/.electron2d/tasks/*.e2task`. На 2026-06-30 в проектную доску перенесены `T-0166`, `T-0221`, `T-0222`, `T-0223` и `T-0225`; корневой `TASKS.md` больше не является источником истины для этих игровых задач. `platformer-acceptance.e2task` остаётся отдельной карточкой человеческой приёмки, но заблокирована до завершения перенесённых задач и не считается принятой.
...
- `.electron2d/tasks/board.e2tasks` - ProjectTaskManager-доска проекта;
- `.electron2d/tasks/platformer-acceptance.e2task` - карточка человеческой приёмки, заблокированная до завершения рабочих задач Platformer;
- `.electron2d/tasks/T-0166.e2task`, `.electron2d/tasks/T-0221.e2task`, `.electron2d/tasks/T-0222.e2task`, `.electron2d/tasks/T-0223.e2task` и `.electron2d/tasks/T-0225.e2task` - рабочие задачи Platformer, перенесённые из корневого списка задач репозитория в проектный `ProjectTaskManager`.

Проект не содержит `TASKS.md`, `data/dev-diary/` или `data/completed-tasks/`: эти файлы относятся только к рабочему процессу разработки самого репозитория и не должны попадать в пользовательские проекты. Для задач игры используется только `ProjectTaskManager` в `.electron2d/tasks/**`.
