...
- читать embedded export presets из `.e2d` project file или loose `export_presets.e2export.json` и проверять точное совпадение target set с релизной матрицей;
...
`tools\Verify-ReferenceGamePlatformMatrix.ps1` проверяет `examples/reference-platformer/` как активный валидный проект `Electron2D.Editor`, который экспортируется из одной кодовой базы под релизную матрицу:
...
Gate не выполняет реальный device smoke и не заменяет 30-минутный soak. Он проверяет подготовленность активного проекта к обычному export workflow: полный набор presets, отсутствие platform-specific игрового fork-а и отсутствие зависимости от локальных workflow-файлов репозитория.
...
- общий `targetSet`;
- разрешённые platform-specific отличия;
- одну активную запись проекта: `reference-platformer`;
...
- проверку, что embedded export presets активного проекта содержат точный target set релиза;
