...
- проходит проектный verifier `tools\Verify-Platformer.ps1`;
...
- запускать `tools\Verify-Platformer.ps1`;
...
`tools\Verify-ReferenceGamePlatformMatrix.ps1` проверяет `examples/platformer/` как активный проект матрицы `Electron2D.Editor`, который объявляет одну кодовую базу и пресеты экспорта под `runtimeTargets`:
...
- запуск `tools\Verify-Platformer.ps1`;
