# Reference game platform matrix 0.1-preview
...
- `releaseVerificationTargets` - платформы, где конкретный релиз `0.1-preview` требует проверяемого результата быстрой и длительной проверки. Для текущего продукта этот набор не сокращает матрицу запуска и экспорта: все шесть `runtimeTargets` остаются в релизной проверке. Источник продуктового решения - раздел «Матрицы платформ» в `docs/releases/0.1-preview.md`; агент не должен удалять iOS или WebAssembly browser из этого набора ради упрощения локальной проверки.
...
Проверяемый результат `T-0096` - C#-проверка `dotnet run --project eng/Electron2D.Build -- verify reference-game-platform-matrix` и артефакт `data/quality/reference-game-platform-matrix.json`. Они должны доказать, что активный reference project:
...
- `releaseVerificationDecision` с явной ссылкой на продуктовое решение, по которому `0.1-preview` пока требует релизную проверку для всех `runtimeTargets`;
- `allowedDifferences` с перечислением разрешённых типов отличий;
- `projects` для активного `platformer`;
...
`dotnet run --project eng/Electron2D.Build -- verify reference-game-platform-matrix` должен:

- запускать `dotnet run --project eng/Electron2D.Build -- verify platformer`;
...
- читать named `.e2d` project file и проверять `format`, `mainScene` и отсутствие ссылок на `TASKS.md`, `data/dev-diary/`, `data/completed-tasks`;
...
`dotnet run --project eng/Electron2D.Build -- verify reference-game-platform-matrix` проверяет `examples/platformer/` как активный проект матрицы `Electron2D.Editor`, который объявляет одну кодовую базу и пресеты экспорта под `runtimeTargets`:
...
```bash
dotnet run --project eng/Electron2D.Build -- verify reference-game-platform-matrix
```

Verifier выполняет:

- запуск `dotnet run --project eng/Electron2D.Build -- verify platformer`;
