...
## Контекст
Electron2D 0.1-preview должен иметь базовый набор runtime controls до начала работ над редактором. Эти controls нужны для игровых HUD, debug-панелей, меню, экранов настроек и будущего редактора, но задача реализует только runtime API библиотеки `Electron2D`.
