...
Связанные документы: [Electron2D 0.1-preview](../releases/0.1-preview.md), [Input event mapping и `InputEvent*`](sdl-input-event-mapping.md).

## Назначение

`0.1-preview` должен дать игре стабильный action-level ввод поверх уже существующих `InputEvent*`. Пользовательский код должен работать с действиями вроде `jump`, `move_left` и `attack`, а не вручную проверять каждую клавишу или кнопку мыши.
...
Electron2D `0.1-preview` получил action-level ввод поверх `InputEvent*`.
