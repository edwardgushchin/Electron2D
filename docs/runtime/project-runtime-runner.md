...
Обновлено: 2026-06-23.
Связанные документы: [Reference platformer](../examples/reference-platformer.md), [UI-heavy reference game](../examples/ui-heavy-reference.md), [Headless runtime automation](headless-runtime-automation.md), [Runtime debug bridge и scene inspection](runtime-debug-bridge.md).
...
- Reference platformer и UI-heavy reference запускаются командой `e2d run --project ...` в режиме разработки или через export player, не содержат `Program.cs`, не вызывают `RuntimeHost`, `ProjectRuntimeRunner` или другой engine-owned bootstrap из user code.
- Reference games не содержат `Console.ReadKey`, ASCII frame output, прямые вызовы `SDL`/window backend или custom event loop.
- `e2d run --project <game> --play-script ... --screenshot <path>` у обеих игр возвращает `WindowCreated=True`, `WindowShown=True`, `FramePresented=True`, `DrawCommands > 0`, `ScreenshotPath=<path>` и создаёт PNG нормального размера.
