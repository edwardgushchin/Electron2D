...
- `data/api/electron2d-api-manifest.json` - generated machine-readable описание public API metadata, созданное из compiled assembly, XML documentation и manual API profile `data/api/electron2d-public-api-profile.json`;
- `data/api/electron2d-public-api-profile.json` - hand-authored source of truth для утверждённых, отложенных и неподдержанных public API types;
...
- `generatedFrom` с путями и hash для API manifest, manual API profile, documentation files и examples file;
...
- `sources` с категориями `apiManifest`, `apiProfile`, `documentation`, `examples`, `wiki`;
...
`sources.wiki` должен быть объектом JSON с полями `generator` и `compatibilityPage`; это ссылка на generated Wiki output, а не источник решений о public API. Строка, массив, `null` или другое значение считаются ошибкой схемы.
...
`data/api/electron2d-public-api-profile.json` остаётся ручным источником решений о включении public API types. `data/api/electron2d-api-manifest.json` остаётся generated источником public API metadata и создаётся из compiled assembly, XML documentation и manual API profile.
