...
Начиная с `T-0034` internal startup policy умеет создать SDL_GPU в Android mobile-compatible profile, выполнить smoke steps и выбрать `Compatibility` fallback либо structured failure по policy.
...
- Integration tests проверяют Android mobile SDL_GPU create options, smoke steps texture/pipeline/command buffer/first submit, `Automatic` fallback, `FailIfUnavailable` и startup log.
