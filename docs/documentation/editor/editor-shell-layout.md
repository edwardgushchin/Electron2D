...
Текущая реализация является model-first: один внутренний `EditorShellLayout` строит стартовый UI root редактора, сохраняемое состояние layout и automated visual harness. Постоянный desktop event loop и содержательная реализация отдельных workspaces добавляются отдельными задачами поверх этого shell.
