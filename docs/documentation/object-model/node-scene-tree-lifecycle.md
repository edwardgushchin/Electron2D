...
`SceneTree` наследуется от `Object` и имеет `Root`. Публичное свойство `Root` остаётся типизированным как `Node`, но фактический root object теперь является `Viewport` с именем `root`.
