...
- Public deferred-call API ещё не реализован.
- `NodePath`, `GetNode()` и `GetNodeOrNull()` реализуются следующей задачей.
- Полная serialization сцены будет добавлена вместе с `PackedScene`/resource pipeline.
