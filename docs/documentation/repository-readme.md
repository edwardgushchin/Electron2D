...
Статус: целевая спецификация для повторной реализации `T-0110` и проверки `T-0213`.
Обновлено: 2026-06-26.
...
В самой таблице состояния записываются как обычный текст без обратных кавычек, чтобы публичный README показывал компактные статусы, а не inline code.
...
iOS и Web показываются в публичном README как запланированные платформы запуска. README не отображает уровень релизной проверки; текущий состав `releaseVerificationTargets` задаётся в `docs/releases/0.1-preview.md`. README не должен превращать Web в отдельный Roadmap-раздел.
...
Целевая проверка после миграции внутреннего инструмента репозитория:
...
- проверять, что видимая tagline отображается ровно один раз: команда учитывает текст `README.md` и только разрешённые брендовые SVG-ассеты, на которые ссылается hero-блок README;
...
- возвращать структурированные JSON diagnostics и ненулевой код завершения при нарушении контракта.

Проверка не должна читать весь репозиторий ради запрещённых формулировок. Исторические, миграционные и аудиторские упоминания в `docs/`, `TASKS.md`, `data/completed-tasks/` или `data/dev-diary/` не относятся к публичному README и не должны ломать `verify readme`.

## Фактическое состояние

Статус: контракт исправлен после rejection предыдущего README; `T-0213` переносит автоматическую проверку в C#-инструмент `eng/Electron2D.Build`.
