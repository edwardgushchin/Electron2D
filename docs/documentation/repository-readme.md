...
Проверка не должна читать весь репозиторий ради запрещённых формулировок. Исторические, миграционные и аудиторские упоминания в `docs/`, `TASKS.md`, `completed-tasks/` или `dev-diary/` не относятся к публичному README и не должны ломать `verify readme`.
