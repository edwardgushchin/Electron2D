# Пользовательская документация 0.1-preview
...
Пользовательская документация должна дать разработчику проверенный путь от установки до запуска первого проекта Electron2D `0.1-preview`. Документация не должна обещать готовые editor workflows, mobile export или другие возможности, которые ещё не имеют проверки в текущем baseline.
...
Команды документации должны ссылаться на существующие C#-проверки репозитория:

- `dotnet run --project eng/Electron2D.Build -- verify project-template` для чистого проекта;
- `dotnet run --project eng/Electron2D.Build -- test` для полного тестового набора;
- `dotnet run --project eng/Electron2D.Build -- package --rid <rid>` для desktop export package checks;
- `dotnet run --project eng/Electron2D.Build -- verify export-documentation` для полноты export guide, platform pages, secret policy и mobile limitations.
...
- Guide ссылается на проверяемые команды, текущие C#-проверки и export guide.
...
Эта страница описывает проверенный baseline Electron2D `0.1-preview`. Она намеренно не выдаёт незавершённые editor, mobile export и production-rendering задачи за готовые возможности.
...
```bash
dotnet add package Electron2D --version 0.1-preview
```

До публикации package используйте локальные C#-проверки репозитория.
...
- Windows x64: `dotnet run --project eng/Electron2D.Build -- package --rid win-x64`;
- Linux x64 glibc: `dotnet run --project eng/Electron2D.Build -- package --rid linux-x64`;
- macOS arm64: `dotnet run --project eng/Electron2D.Build -- package --rid osx-arm64` на macOS host.
...
- запустите `dotnet run --project eng/Electron2D.Build -- verify project-template`;
