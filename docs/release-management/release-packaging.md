...
Этот файл является доменным документом для будущего release packaging pipeline. Документ фиксирует целевой контракт после отказа от PowerShell-автоматизации; текущая задача `T-0111` остаётся заблокированной до появления C# repository tool.
...
Статус: целевая спецификация для `T-0111`, зависит от `T-0209` и `T-0210`.
Обновлено: 2026-06-24.
...
Статус: заблокировано до C# repository tool.

В репозитории всё ещё есть PowerShell-based automation, поэтому `T-0111` не может считаться готовой. Предыдущий PowerShell rehearsal layer был отклонён и не является целевым решением.
