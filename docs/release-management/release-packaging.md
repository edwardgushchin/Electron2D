...
Проверка должна подтверждать отсутствие отслеживаемых `.ps1`-скриптов, шагов рабочего процесса с `pwsh` и PowerShell-команд в активных рабочих путях, CI, входных данных релизной сборки и ссылках на рабочие процессы в задачах и документах. Исторические заметки, миграционные документы и заметки об отказах допустимы только как точные записи C#-разрешающего списка.

Для разбора оставшихся разрешённых строк используется отдельный диагностический вызов:

```bash
dotnet run --project eng/Electron2D.Build -- verify no-powershell-workflows allowed-mentions
```

Он выводит `path`, `lineNumber` и `message` для каждого разрешённого исторического, миграционного или отказного упоминания. Если в той же области найден активный рабочий путь PowerShell, вызов выводит ошибку и возвращает ненулевой код завершения. Этот вызов не заменяет строгую проверку `verify no-powershell-workflows` и не объявляет PowerShell допустимым рабочим путём.
