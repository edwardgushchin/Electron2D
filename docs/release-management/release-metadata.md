...
Task state не является релизным черновиком и хранится в `.taskboard/**`. Проверка release metadata не классифицирует taskboard как release draft; исключение `.taskboard/**`, legacy `TASKS.md`, `data/completed-tasks/` и `data/dev-diary/` из пользовательских экспортов и релизных архивов проверяется соответствующими project-template, export и package правилами.
...
- `.taskboard/**` остаётся tracked task state репозитория по правилам `AGENTS.md` и не проверяется как релизный черновик.
