# Версионирование и release metadata `0.1-preview`
...
`TASKS.md` не является релизным черновиком: это отслеживаемый Git рабочий журнал активных задач репозитория. Проверка release metadata не должна запрещать tracked `TASKS.md`; исключение `TASKS.md`, `data/completed-tasks/` и `data/dev-diary/` из пользовательских шаблонов, экспортов и релизных архивов проверяется соответствующими project-template, export и package правилами.
...
Verifier сверяет package metadata в `src/Electron2D/Electron2D.csproj`, иконку `Electron2D.Editor`, наличие брендовых файлов, ссылки README на брендовые SVG, отсутствие tracked release draft файлов `CHANGELOG*` и `RELEASE-NOTES*`, а также упоминание package version `0.1-preview` в README. Команда выполняется C#-инструментом репозитория и выводит структурированные JSON-диагностики. Для `T-0214` `verify release-metadata` является единственной целевой поверхностью проверки release metadata.
...
- `TASKS.md` остаётся tracked task tracker репозитория по правилам `AGENTS.md` и не проверяется как релизный черновик.
...
Команда проверяет согласованность package metadata, README, брендовых файлов, иконки `Electron2D.Editor` и отсутствие tracked release draft файлов `CHANGELOG*` и `RELEASE-NOTES*`.
