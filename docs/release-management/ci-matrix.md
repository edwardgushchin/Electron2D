...
- `verify no-powershell-workflows` проверяет tracked production paths: workflow-файлы, `AGENTS.md`, локальные навыки, активные доменные документы, `TASKS.md`, generated documentation metadata, `data/quality` и бывший корневой путь `tools/`, если такой путь снова появится в Git. Команда fail-closed отклоняет tracked `tools/*.ps1`, `pwsh`, `.ps1` и команды PowerShell как активный workflow; untracked drafts не входят в этот контракт, а исторические, миграционные и rejection-упоминания допускаются только через явный разрешающий список.
...
`verify no-powershell-workflows` является финальным контролем `T-0210`: workflow, инструкции и активные документы не должны возвращать PowerShell как рабочий путь.
...
- `verify no-powershell-workflows` возвращает `E2D-BUILD-NO-POWERSHELL-WORKFLOWS-PASSED`, если tracked production paths не содержат active PowerShell workflow.
