...
- `verify release-metadata` проверяет package metadata, README, брендовые файлы и отсутствие отслеживаемых локальных релизных черновиков `CHANGELOG*` и `RELEASE-NOTES*` через C#-код; `.taskboard/**` остаётся отдельным task state репозитория.
