...
Headless-сессия открывает `*.scene.json` и `.taskboard/*.e2task`, чтобы job snapshot identity и task tools работали с теми же document revisions, что и active Editor route.
