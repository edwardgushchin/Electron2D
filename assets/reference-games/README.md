...
Verifier читает `assets/reference-games/manifest.json`, проверяет наличие всех файлов, SHA-256, размер, разрешённые расширения, локальность путей, наличие license metadata и обязательные роли для `reference-platformer` и `ui-heavy-reference`.
