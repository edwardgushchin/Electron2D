...
/// <summary>
/// Форматирование <see cref="ProfilerFrame"/> в человекочитаемый моноширинный текст (ASCII/Unicode box).
/// </summary>
public static class ProfilerFrameFormatting
{
    private const string InvalidFrameText = "<ProfilerFrame: invalid>";

    static ProfilerFrameFormatting() => Console.OutputEncoding = Encoding.UTF8;

    /// <summary>
    /// Возвращает “pretty” строку со сводкой по кадру профайлера.
    /// </summary>
    /// <param name="f">Снапшот кадра.</param>
    /// <param name="includeZeros">
    /// Если <see langword="true"/>, нулевые тайминги будут выводиться как числа (например, 0.00),
    /// иначе часть значений будет заменяться на “—”.
    /// </param>
    public static string ToPrettyString(this ProfilerFrame f, bool includeZeros = false)
    {
        if (!f.IsValid)
            return InvalidFrameText;
        ...
        Row3(sb,
            MsCell("EventsPump", f.EventsPumpMs),
            MsCell("InputPoll", f.InputPollMs),
            MsCell("EventsSwap", f.EventsSwapMs));
        Row3(sb,
            MsCell("HandleQuit", f.HandleQuitCloseMs),
            MsCell("DispatchInput", f.DispatchInputMs),
            MsCell("FlushFreeQ", f.FlushFreeQueueMs));
        Row3(sb,
            MsCell("FixedStep", f.FixedStepMs),
            MsCell("Process", f.ProcessMs),
            MsCell("RenderTotal", f.RenderTotalMs));

        sb.Append(Mid("Render timings (ms)"));
        Row3(sb,
            MsCell("BeginFrame", f.RenderBeginFrameMs),
            MsCell("BuildQueue", f.RenderBuildQueueMs),
            MsCell("Sort", f.RenderSortMs));
        Row3(sb,
            MsCell("Flush", f.RenderFlushMs),
            MsCell("Present", f.RenderPresentMs),
            MsCell("FrameTotal", f.FrameMs));
        ...
        static string Kb(long bytes, CultureInfo invCulture)
            => (bytes / 1024.0).ToString("F1", invCulture);
        ...
        static void Row3(StringBuilder sb, string c1, string c2, string c3)
        {
        ...
        static string Border(char left, char right, string title)
        {
            var t = " " + title + " ";
        ...
        static string Top(string title) => Border('┌', '┐', title);

        static string Mid(string title) => Border('├', '┤', title);

        static string Bot() => "└" + new string('─', InnerWidth) + "┘";

        void Row2(StringBuilder sb, string c1, string c2)
        {
            // Третий столбец заполняем пробелами для фиксированной ширины.
        ...
        {
            // Счётчики часто полезнее видеть как 0, чем как "—".
