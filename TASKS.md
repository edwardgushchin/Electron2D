...
- [ ] Для исправительной итерации с непустым `metadata.previousVerdictChain` каждый `metadata.blockerClosureList` должен быть связан с фактическим `checks[].name` текущего package config; свободная фраза "blocker закрыт" без локальной проверки не считается готовностью к новой внешней отправке.
