...
Обновлено: 2026-07-05T02:09:00+03:00.

Этот раздел задаёт рекомендуемый порядок выполнения только активных задач из текущего `TASKS.md`. Закрытые задачи, архивная история и внешние project-board карточки сюда не попадают; они остаются в `data/completed-tasks/**`, дневниках или в своих досках.

### 0. Правила чтения

- Roadmap не создаёт новые задачи и не заменяет поля `Зависимости`.
- Каждый `T-*`, перечисленный ниже, должен иметь активный раздел выше в этом файле.
- Если задача заблокирована внешней доской, окружением или платформенным prerequisite, roadmap называет блокер словами, но не перечисляет неактивные или внешние task IDs как шаги выполнения.
- Архивные audit-loop задачи больше не перечисляются как шаги выполнения; их правила учитываются через актуальные документы, команды проверки и оставшиеся активные follow-up задачи.

### 1. Ближайшая очистка release tooling

1. `T-0239` — удалить неактивный screenshot recorder из внутренней реализации `audit submit`.

Причина приоритета: это небольшой остаточный долг после принятого audit workflow. Его лучше закрыть до следующей большой задачи с внешним аудитом, чтобы не тащить мёртвую диагностику в новые пакеты.

### 2. Публичный texture API

1. `T-0226` — привести базовый `Texture2D` к выбранному контракту Godot 4.7: `Image`, формат, placeholder resource и методы отрисовки.
2. `T-0227` — после `T-0226` привести `AtlasTexture` к тому же контракту и заменить одноуровневое разрешение texture resource рекурсивным механизмом.

Эта ветка независима от редакторной оболочки и может идти параллельно ранним UI-задачам, если не мешает локальным проверкам.

### 3. Editor Foundation

1. `T-0175` — устранить конфликт Project Settings и canonical `.e2d`; нужно до задач, которые сохраняют или читают project settings через UI.
2. `T-0172` — держать как активный трек runtime UI gaps: применять, когда очередная UI-задача выявляет недостающий общий runtime-механизм.
3. `T-0174` — усилить window smoke diagnostics; можно делать рядом с ранней UI foundation, потому это снижает стоимость отладки окна.
4. `T-0176` -> `T-0177` -> `T-0178` — базовая тема, текст/scrolling/DPI, interaction/focus/pointer lifecycle.
5. После `T-0178`: параллельно `T-0194`, `T-0195`, `T-0196`.
6. После `T-0196`: `T-0197`.
7. После desktop control foundation: `T-0179`, затем `T-0180`.
8. Product shell path: `T-0181` -> `T-0182` -> `T-0183`.

### 4. Editor Product Workspaces

После `T-0183` вести отдельными потоками, но не закрывать `T-0189`, пока нужные потоки не прошли свои integration gates.

1. Scene editing: `T-0190`, `T-0191`, `T-0192`, затем `T-0193`.
2. Script workspace: `T-0198` -> `T-0199`/`T-0200` -> `T-0201`, затем закрыть `T-0184`.
3. Specialized editors: `T-0202`/`T-0203`/`T-0204` -> `T-0205`, затем закрыть `T-0187`.
4. Supporting workspaces после `T-0183`: `T-0185`, `T-0186`, `T-0188`.
5. После закрытия нужных product-workspace потоков: `T-0189`.

### 5. Public Site And Release Packaging

1. `T-0110` — переписать публичный README как страницу продукта.
2. `T-0111` — после `T-0110` подготовить cross-platform release packaging и draft GitHub Release workflow.

Эта ветка нужна до release candidate gate, но не должна блокировать раннюю editor/runtime разработку.

### 6. Preview Release Gates

1. `T-0092` — iOS arm64 export остаётся environment-gated: без macOS/Xcode, iOS simulator или устройства задача не закрывается.
2. `T-0093` — Tier 1 platform smoke/soak после закрытия обязательных платформенных и preview-project prerequisites.
3. `T-0104` — release candidate gate после `T-0093`, `T-0111` и `T-0189`.
4. `T-0105` — post-preview risk register после `T-0104`.
