...
- [ ] Создана и закрыта tracking-задача `T-0208`.
...
## T-0208 [ ] P0: Перенести тестовые, документационные и API verifiers на C#
...
- Зависимости: T-0207
- Ссылки:
  - Доменный документ: `docs/documentation/repository-readme.md`; `docs/documentation/local-documentation-pipeline.md`; `docs/release-management/api-compatibility.md`
  - Будущий исходный код: `eng/Electron2D.Build/`
  - Тесты: `tests/Electron2D.Tests.Integration/`

### Самодостаточное описание

Tracking-задача для переноса тестовых, документационных и API-related verifier-ов в C# commands внутреннего build tool. Собственный production scope разбит на дочерние задачи `T-0213`, `T-0214` и `T-0215`, чтобы README/docs, API/Wiki/license/manifests и test/performance runners можно было выполнять автономно.

### Критерии приёмки

- [x] Создана и закрыта `T-0213`.
- [x] Создана и закрыта `T-0214`.
- [ ] Создана и закрыта `T-0215`.
- [ ] Доменные документы и active tasks больше не требуют PowerShell-команды как целевой verifier для README, docs, API, Wiki, license, manifests, tests и performance checks.
- [ ] После закрытия дочерних задач общий C# verification entrypoint проходит на Windows, Linux и macOS runners.
...
## T-0233 [ ] P0: Разграничить роли `AGENTS.md`, `AUDIT-REQUEST.md` и `goal-task-loop.md`
...
- Ссылки:
  ...
  - Локальный prompt оркестратора: `G:\Projects\2026\Electron2D\.codex\prompts\goal-task-loop.md`
  - Доменный документ: `docs/release-management/audit-package.md`

### Самодостаточное описание

Три файла сейчас частично дублируют и конфликтно описывают внешний аудит. `AGENTS.md` должен задавать обязательные правила работы агента внутри репозитория: безопасность, task workflow, дневник, проверки, упаковку и штатную отправку через C#-команду. `AUDIT-REQUEST.md` должен быть только текстом для внешнего аудитора: что проверять в одном audit ZIP и какой финальный отчёт вернуть. `goal-task-loop.md` должен быть локальным prompt-ом оркестратора: как вести задачу до внешнего `ACCEPT`, но без ручных браузерных инструкций, без закрытия чужих вкладок и без альтернативных способов чтения отчёта.

Нужное поведение: границы между файлами должны быть явно зафиксированы, а устаревшие правила в `goal-task-loop.md` должны быть заменены ссылкой на C#-инструмент и доменный документ. В частности, prompt оркестратора не должен предписывать переиспользовать или закрывать пользовательские вкладки, вручную открывать ChatGPT, писать аудитору служебный текст вроде `<task-id> audit rNN`, брать отчёт через кнопку копирования ответа, читать общий DOM страницы или обновлять страницу вручную. Штатный путь внешнего аудита должен оставаться один: `audit package`, `audit package verify`, затем `audit submit --browser-backend codex-chrome`.

### Критерии приёмки

- [ ] `AGENTS.md` описывает только правила для агента и ссылается на `docs/release-management/audit-package.md` как источник точной механики внешнего аудита.
- [ ] `AGENTS.md` не содержит ручного браузерного сценария, который дублирует или обходит C#-команду `audit submit`.
- [ ] `AUDIT-REQUEST.md` остаётся запросом внешнему аудитору и не содержит инструкций про Chrome, вкладки, cookies, operator workflow, локальную проверку ZIP, контрольные суммы или ручное скачивание отчёта.
- [ ] `goal-task-loop.md` описывает orchestration loop: preflight, worker, checks, package, verify, submit, verdict, fix loop, accept and commit.
- [ ] `goal-task-loop.md` делегирует всю механику браузера, режима `Глубокое исследование`, прикрепления ZIP, обновления страницы, скачивания Markdown и закрытия собственной вкладки команде `audit submit`.
- [ ] `goal-task-loop.md` и `AGENTS.md` явно фиксируют, что реализация любой задачи проходит внешний audit loop через `audit submit` до `VERDICT: ACCEPT`; задача не закрывается и не считается принятой без успешного внешнего отчёта.
- [ ] `goal-task-loop.md` не требует закрывать или переиспользовать чужие вкладки ChatGPT.
- [ ] `goal-task-loop.md` не запрещает вести `TASKS.md` и дневник в случаях, где это требует `AGENTS.md`; вместо этого он запрещает только выдуманные или неподтверждённые служебные факты.
- [ ] Команды в `goal-task-loop.md` используют переносимые пути с `/` для аргументов audit tool, чтобы не повторять ошибку `--out contains a backslash`.
- [ ] `goal-task-loop.md` не превышает 4000 символов, чтобы помещаться в режим goal для Codex.
- [ ] Доменный документ `docs/release-management/audit-package.md` остаётся единственным подробным источником контракта C#-инструмента отправки.
- [ ] После правок выполнены `git diff --check` и релевантная проверка документации, если изменялись tracked docs.

### Подзадачи

- [ ] Сравнить текущие тексты `AGENTS.md`, `AUDIT-REQUEST.md`, `goal-task-loop.md` и `docs/release-management/audit-package.md`.
- [ ] Удалить из `goal-task-loop.md` устаревшие ручные browser/audit/wait/read правила.
- [ ] Оставить в `goal-task-loop.md` только orchestration-level правила и ссылки на C#-инструмент.
- [ ] Проверить, что `AUDIT-REQUEST.md` не смешивает content audit с operator workflow.
- [ ] Проверить, что `AGENTS.md` не дублирует низкоуровневые клики и не конфликтует с доменным документом.
- [ ] Запустить минимальные проверки и зафиксировать результат.

### Заметки агента

2026-06-29T17:08:00+03:00 - Создано по запросу пользователя после повторяющейся ошибки с backslash в аргументах `audit package` и обнаруженного конфликта между `goal-task-loop.md`, `AGENTS.md` и текущим C#-контрактом внешнего аудита.

2026-06-29T17:15:00+03:00 - Пользователь уточнил дополнительное ограничение: `goal-task-loop.md` должен быть не длиннее 4000 символов, иначе он не помещается в режим goal для Codex. Ограничение добавлено в критерии приёмки `T-0233`.

2026-06-29T17:18:00+03:00 - Пользователь уточнил обязательный общий gate: для реализации любой задачи должен использоваться внешний цикл `audit submit` до `VERDICT: ACCEPT`. Это добавлено в критерии приёмки `T-0233` как требование к согласованию `AGENTS.md` и `goal-task-loop.md`.

2026-06-29T20:17:00+03:00 - Пользователь явно попросил добавить `AGENTS.md`, `.codex/prompts/goal-task-loop.md`, `TASKS.md`, `completed-tasks/` и `dev-diary/` в отслеживание Git и сделать коммит. Правила `.gitignore` и `AGENTS.md` обновляются отдельно от полного закрытия `T-0233`: задача остаётся открытой до сокращения и смыслового согласования `goal-task-loop.md` с C#-командой `audit submit`.

## ROADMAP

Обновлено: 2026-06-29T17:08:00+03:00.
