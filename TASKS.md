...
- [ ] `.codex/prompts/goal-prompt.md` является коротким active goal prompt-ом с лимитом 3500 символов, а `.codex/prompts/goal-task-workflow.md` является подробной workflow-моделью; оба файла должны быть отслеживаемыми репозиторными артефактами и проверяться `verify audit-contracts`.
...
- [ ] `.codex/prompts/goal-task-workflow.md` требует записывать в дневник числовую строку по уровням, например `Fast: 3 теста / 1m13s; Medium: 4 теста / 3m18s; Heavy: не запускался`, вместо общей фразы "точечные тесты прошли". Для аудиторской автоматизации записи без количества проверок и времени недостаточны.
